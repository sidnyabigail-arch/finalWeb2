...
const AdminDashboard = () => {
  const [activeTab, setActiveTab] = useState('clientes'); // Abierto en clientes para que veas el cambio
  ...
  const fetchAllData = async () => {
    ...
    // Obtenemos los usuarios y filtramos solo a los clientes
  ...
  /* --- GESTIÓN DE ESTADO DE CLIENTES (NUEVO) --- */
  const changeClientStatus = async (clientId, newStatus) => {
    try {
      await update(ref(db, `users/${clientId}`), { status: newStatus });
      fetchAllData(); // Refrescamos la tabla inmediatamente
    } catch (error) {
  ...
  return (
    <div className="container" style={{ padding: '40px 20px', minHeight: '80vh' }}>
      ...
      <div style={styles.tableContainer}>
        ...
        {/* TABLA CLIENTES ACTUALIZADA CON CONTROL DE ESTADO */}
        {activeTab === 'clientes' && (
          <table style={styles.table}>
            <thead><tr><th style={styles.th}>Nombre</th><th style={styles.th}>Email</th><th style={styles.th}>Registro</th><th style={styles.th}>Control de Acceso</th></tr></thead>
            <tbody>
                ...
                <tr key={c.id} style={styles.tr}>
                  ...
                  <td style={styles.td}>
                    <select 
                      ...
                      onChange={(e) => changeClientStatus(c.id, e.target.value)} 
                      style={{
                        ...styles.statusSelect, 
                        backgroundColor: (c.status || 'activo') === 'activo' ? '#dcfce7' : '#fee2e2',
                        color: (c.status || 'activo') === 'activo' ? '#166534' : '#b91c1c'
                      }}
                    >
                      <option value="activo">✓ Cuenta Activa</option>
                      <option value="inactivo">🚫 Cuenta Bloqueada</option>
                    </select>
                  </td>
                </tr>
              ))}
      ...
      <Modal isOpen={isProductModalOpen} onClose={() => setIsProductModalOpen(false)} title={editingId ? 'Editar Producto' : 'Nuevo Producto'}>
        <form onSubmit={saveProduct} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
          ...
          <label style={{ fontSize: '12px', fontWeight: 'bold' }}>Categoría (Opcional)</label>
          ...
          <div style={{ backgroundColor: '#f9fafb', padding: '15px', borderRadius: '8px', border: '1px solid #eee' }}>
            ...
            {formData.images.map((img, index) => (
              <div key={index} style={{ marginBottom: '10px' }}>
                <input type="text" placeholder="Pegar URL de la imagen..." value={img} onChange={(e) => handleImageChange(index, e.target.value, false)} style={{...styles.input, width: '100%', marginBottom: '5px'}} />
                <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                  <input type="file" accept="image/*" onChange={(e) => handleFileUpload(index, e, false)} style={{ fontSize: '12px' }} />
                  {formData.images.length > 1 && <button type="button" onClick={() => setFormData({...formData, images: formData.images.filter((_, i) => i !== index)})} style={styles.deleteBtn}>X</button>}
                </div>
                {img && <img src={img} alt="preview" style={{ width: '60px', height: '60px', objectFit: 'cover', borderRadius: '4px', marginTop: '5px' }} />}
              </div>
            ))}
            <button type="button" onClick={() => setFormData({...formData, images: [...formData.images, '']})} style={{ background: 'none', color: 'var(--primary-pink)', border: '1px solid var(--primary-pink)', padding: '5px 10px', borderRadius: '6px', fontSize: '12px', cursor: 'pointer' }}>+ Agregar otra imagen</button>
          </div>
          ...
          <div style={{ backgroundColor: '#f9fafb', padding: '15px', borderRadius: '8px', border: '1px solid #eee' }}>
            ...
            {promoFormData.images.map((img, index) => (
              <div key={index} style={{ marginBottom: '15px', paddingBottom: '10px', borderBottom: '1px dashed #ccc' }}>
                <input type="text" placeholder="Pegar URL de la imagen..." value={img} onChange={(e) => handleImageChange(index, e.target.value, true)} style={{...styles.input, width: '100%', marginBottom: '5px'}} />
      ...
      <Modal isOpen={isGuideModalOpen} onClose={() => setIsGuideModalOpen(false)} title={editingId ? 'Editar Artículo' : 'Nuevo Artículo Floral'}>
        <form onSubmit={saveGuide} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
          ...
          <div style={{ backgroundColor: '#f9fafb', padding: '15px', borderRadius: '8px', border: '1px solid #eee' }}>
            ...
            <input type="text" placeholder="O pega la URL aquí..." value={guideFormData.image} onChange={(e) => setGuideFormData({...guideFormData, image: e.target.value})} style={{...styles.input, marginBottom: '10px'}} />
        ...
        {viewItem && (viewItem.type === 'product' || viewItem.type === 'promotion') && (
          <div>
            ...
            <p>
              ...
              {viewItem.promoTag && <span style={{ marginLeft: '10px', backgroundColor: '#e11d48', color: 'white', padding: '3px 8px', borderRadius: '10px', fontSize: '11px' }}>{viewItem.promoTag}</span>}
        ...
        {viewItem && viewItem.type === 'order' && (
          <div style={{ padding: '10px' }}>
              ...
              <div style={{ flex: 1, minWidth: '250px', backgroundColor: '#f8fafc', padding: '20px', borderRadius: '12px', border: '1px solid #eee', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                ...
                {viewItem.payment?.method === 'qr' ? (
                  ...
                  <div style={{ width: '100%', backgroundColor: 'white', padding: '15px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                    <p style={{ margin: '0 0 10px 0', fontWeight: 'bold', color: '#2563eb' }}>Tarjeta de Crédito/Débito</p>
                    <p style={{ margin: '5px 0', fontSize: '14px' }}><b>Titular:</b> {viewItem.payment?.cardName}</p>
                    <p style={{ margin: '5px 0', fontSize: '14px' }}><b>Tarjeta terminada en:</b> **** {viewItem.payment?.cardLast4}</p>
                    <div style={{ marginTop: '15px', padding: '10px', backgroundColor: '#dcfce7', color: '#166534', borderRadius: '6px', textAlign: 'center', fontWeight: 'bold', fontSize: '14px' }}>✓ Procesado</div>
                  </div>
                )}
              </div>
            </div>
            ...
            <div style={{ marginTop: '25px' }}>
              <h4 style={{ color: 'var(--text-dark)', borderBottom: '1px solid #eee', paddingBottom: '10px', marginBottom: '15px' }}>📦 Productos Solicitados</h4>
